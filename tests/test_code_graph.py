...
_EMBEDDING_ENVIRONMENT_VARIABLES = (
    "LANGCODE_ACCEPT_CLOUD_EMBEDDINGS",
    "LANGCODE_EMBEDDING_BASE_URL",
    "LANGCODE_EMBEDDING_MODEL",
    "LANGCODE_EMBEDDING_API_KEY",
)
...
class CodeGraphBehaviorTests(unittest.TestCase):
    ...
    def call(self, **inp):
        with patch("pathlib.Path.home", return_value=self.home):
            raw = asyncio.run(CodeGraphService(self.repo).execute(inp))
    ...
    def test_semantic_search_requires_explicit_cloud_egress_consent(self):
        ...
        with FakeEmbeddingServer(lambda _text: [1.0]) as server:
            environment = embedding_environment(
                LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                LANGCODE_EMBEDDING_MODEL="test-model",
            )
    ...
    def test_embedding_configuration_ignores_target_repository_dotenv(self):
        ...
        (self.repo / ".env").write_text(
            ...
            "LANGCODE_EMBEDDING_BASE_URL=https://invalid.example/v1\n"
            "LANGCODE_EMBEDDING_MODEL=parent-model\n",
            encoding="utf-8",
        )
        self.commit_all()
    ...
    def test_semantic_search_embeds_structure_and_ranks_by_cosine_similarity(self):
        ...
        with FakeEmbeddingServer(vector_for) as server:
            environment = embedding_environment(
                ...
                LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                LANGCODE_EMBEDDING_MODEL="test-model",
                LANGCODE_EMBEDDING_API_KEY="secret-key",
            )
            with patch.dict(os.environ, environment, clear=True):
                result = self.call(
                    action="search",
    ...
    def test_semantic_search_reuses_cache_and_refreshes_only_changed_file_nodes(self):
        ...
        with FakeEmbeddingServer(lambda _text: [1.0, 0.0]) as server:
            environment = embedding_environment(
                LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                LANGCODE_EMBEDDING_MODEL="cache-model",
            )
            with patch.dict(os.environ, environment, clear=True):
                first = self.call(
    ...
    def test_hybrid_search_merges_fts_and_semantic_candidates_with_context_boost(self):
        ...
        with FakeEmbeddingServer(vector_for) as server:
            environment = embedding_environment(
                LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                LANGCODE_EMBEDDING_MODEL="hybrid-model",
            )
            with patch.dict(os.environ, environment, clear=True):
                merged = self.call(
    ...
    def test_semantic_fails_strictly_and_hybrid_falls_back_on_provider_error(self):
        ...
        with FakeEmbeddingServer(
            ...
            environment = embedding_environment(
                LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                LANGCODE_EMBEDDING_MODEL="failing-model",
            )
            with patch.dict(os.environ, environment, clear=True):
                semantic = self.call(
    ...
    def test_missing_provider_config_is_strict_for_semantic_and_warns_for_hybrid(self):
        ...
        with patch.dict(
            os.environ,
            embedding_environment(LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1"),
            clear=True,
        ):
            semantic = self.call(
                action="search", query="process_order", mode="semantic"
    ...
    def test_invalid_embedding_endpoint_returns_embedding_error(self):
        ...
        environment = embedding_environment(
            LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
            LANGCODE_EMBEDDING_BASE_URL="not-a-url",
            LANGCODE_EMBEDDING_MODEL="test-model",
        )
        with patch.dict(os.environ, environment, clear=True):
            result = self.call(
    ...
    def test_embedding_transport_failures_are_mapped_and_hybrid_falls_back(self):
        ...
        environment = embedding_environment(
            LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
            LANGCODE_EMBEDDING_BASE_URL="http://127.0.0.1:9/v1",
            LANGCODE_EMBEDDING_MODEL="transport-model",
        )
        failures = (
        ...
        for failure in failures:
            with self.subTest(failure=type(failure).__name__):
                with patch.dict(os.environ, environment, clear=True):
                    with patch("langchain_openai.OpenAIEmbeddings.embed_documents", side_effect=failure):
                        semantic = self.call(
                            action="search", query="searchable", mode="semantic"
                        )
                        hybrid = self.call(
    ...
    def test_embedding_retries_transient_errors_and_rejects_invalid_responses(self):
        ...
        with FakeEmbeddingServer(lambda _text: [1.0], retry_responder) as server:
            environment = embedding_environment(
                LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                LANGCODE_EMBEDDING_MODEL="retry-model",
            )
            with patch.dict(os.environ, environment, clear=True):
                retried = self.call(
                    action="search", query="searchable", mode="semantic"
            ...
            with self.subTest(response=name):
                with FakeEmbeddingServer(lambda _text: [1.0], responder) as server:
                    environment = embedding_environment(
                        LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                        LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                        LANGCODE_EMBEDDING_MODEL=f"invalid-{name}",
                    )
                    with patch.dict(os.environ, environment, clear=True):
                        result = self.call(
                            action="search", query="searchable", mode="semantic"
    ...
    def test_embedding_caches_are_provider_scoped_and_drop_deleted_symbols(self):
        ...
        with FakeEmbeddingServer(lambda _text: [1.0, 0.0]) as first_server:
            base_environment = embedding_environment(
                LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                LANGCODE_EMBEDDING_BASE_URL=first_server.base_url,
            )
            with patch.dict(
                os.environ,
                {**base_environment, "LANGCODE_EMBEDDING_MODEL": "model-a"},
                clear=True,
            ):
                first = self.call(
                    action="search", query="symbol", mode="semantic"
            ...
            with patch.dict(
                os.environ,
                {**base_environment, "LANGCODE_EMBEDDING_MODEL": "model-b"},
                clear=True,
            ):
            ...
            with FakeEmbeddingServer(lambda _text: [1.0, 0.0]) as second_server:
                second_environment = embedding_environment(
                    LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                    LANGCODE_EMBEDDING_BASE_URL=second_server.base_url,
                    LANGCODE_EMBEDDING_MODEL="model-a",
                )
    ...
    def test_embedding_batches_commit_progress_and_resume_after_failure(self):
        ...
        with FakeEmbeddingServer(lambda _text: [1.0, 0.0], responder) as server:
            environment = embedding_environment(
                LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                LANGCODE_EMBEDDING_MODEL="batch-model",
            )
            with patch.dict(os.environ, environment, clear=True):
                failed = self.call(
                    action="search", query="symbol", mode="semantic"
    ...
    def test_non_semantic_actions_never_call_the_embedding_provider(self):
        ...
        with FakeEmbeddingServer(lambda _text: [1.0]) as server:
            environment = embedding_environment(
                LANGCODE_ACCEPT_CLOUD_EMBEDDINGS="1",
                LANGCODE_EMBEDDING_BASE_URL=server.base_url,
                LANGCODE_EMBEDDING_MODEL="unused-model",
            )
            with patch.dict(os.environ, environment, clear=True):
    ...
    def test_indexes_only_supported_language_extensions_and_relationships(self):
        fixtures = {
            "sample.py": "from helpers import py_helper\n\ndef py_run():\n    return py_helper()\n",
            "helpers.py": "def py_helper():\n    return 1\n",
            "sample.c": '#include "sample.h"\n\nint c_helper() { return 1; }\nint c_run() { return c_helper(); }\n',
            "sample.h": "int c_helper(void);\n",
            "sample.cc": "int cc_run() { return 1; }\n",
            "sample.cpp": "class CppBase {};\nclass CppType : public CppBase { public: int cpp_run() { return 1; } };\n",
            "sample.cxx": "int cxx_run() { return 1; }\n",
            "sample.hh": "int hh_run() { return 1; }\n",
            "sample.hpp": "int hpp_run() { return 1; }\n",
        }
        ...
        self.assertEqual(overview["data"]["languages"], {"c": 2, "cpp": 5, "python": 2})
        expected_symbols = [
            "py_run", "py_helper", "c_helper", "c_run", "cc_run", "CppBase",
            "CppType", "cpp_run", "cxx_run", "hh_run", "hpp_run",
        ]
        ...
        callers = self.call(action="query", relation="callers_of", target="c_helper")
        self.assertEqual(
            [item["qualified_name"] for item in callers["data"]["results"]],
            ["sample.c::c_run"],
        )
        importers = self.call(action="query", relation="importers_of", target="sample.h")
        self.assertEqual(
            [item["qualified_name"] for item in importers["data"]["results"]],
            ["sample.c"],
        )
        inheritors = self.call(action="query", relation="inheritors_of", target="CppBase")
        self.assertEqual(
            [item["qualified_name"] for item in inheritors["data"]["results"]],
            ["sample.cpp::CppType"],
        )

    def test_indexes_overloaded_cpp_methods_without_collisions(self):
        (self.repo / "Overloads.cpp").write_text(
            "class Overloads {\n"
            "public:\n"
        ...
        cpp = self.call(action="search", query="convert", max_results=10)
        self.assertTrue(cpp["ok"], cpp)
        self.assertEqual(len(cpp["data"]["results"]), 2)
        self.assertEqual(
            len({item["qualified_name"] for item in cpp["data"]["results"]}),
            2,
        )
        ambiguous = self.call(
            action="query", relation="callers_of", target="convert"
        )
        self.assertEqual(ambiguous["error"]["code"], "ambiguous_target")

    def test_unsupported_language_files_are_ignored_by_index_and_impact(self):
        (self.repo / "app.py").write_text(
            "def supported_symbol():\n    return 1\n", encoding="utf-8"
        )
        unsupported = {
            "ignored.js": "function javascript_symbol() {}\n",
            "ignored.ts": "function typescript_symbol() {}\n",
            "ignored.tsx": "function tsx_symbol() {}\n",
            "ignored.java": "class JavaSymbol {}\n",
            "ignored.go": "package ignored\nfunc go_symbol() {}\n",
            "ignored.rs": "fn rust_symbol() {}\n",
            "ignored.cs": "class CSharpSymbol {}\n",
        }
        for name, content in unsupported.items():
            (self.repo / name).write_text(content, encoding="utf-8")
        self.commit_all()
        ...
        for symbol in (
            "javascript_symbol", "typescript_symbol", "tsx_symbol", "JavaSymbol",
            "go_symbol", "rust_symbol", "CSharpSymbol",
        ):
            with self.subTest(symbol=symbol):
                result = self.call(action="search", query=symbol)
                self.assertEqual(result["data"]["results"], [], result)

        (self.repo / "ignored.ts").write_text(
            "function changed_typescript_symbol() {}\n", encoding="utf-8"
        )
        impact = self.call(action="impact")
        self.assertEqual(impact["data"]["changed_files"], [])

    def test_schema_upgrade_rebuilds_index_and_clears_legacy_cache(self):
        (self.repo / "app.py").write_text(
            "def supported_symbol():\n    return 1\n", encoding="utf-8"
        )
        self.commit_all()
        ...
        with patch("pathlib.Path.home", return_value=self.home):
            from lib.code_graph import _cache_path
            from lib._code_graph_embeddings import _ensure_schema

            cache_path = _cache_path(self.repo)
            conn = sqlite3.connect(cache_path)
            conn.row_factory = sqlite3.Row
            conn.execute("PRAGMA foreign_keys=ON")
            _ensure_schema(conn)
            conn.execute(
                "INSERT INTO files(path, language, size, mtime_ns, content_hash) "
                "VALUES(?, ?, ?, ?, ?)",
                ("ignored.js", "javascript", 1, 1, "legacy-file-hash"),
            )
            conn.execute(
                "INSERT INTO nodes(file_path, kind, name, qualified_name, "
                "parent_qualified, line_start, line_end, is_test) "
                "VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    "ignored.js", "function", "LegacyJs", "ignored.js::LegacyJs",
                    "ignored.js", 1, 1, 0,
                ),
            )
            conn.execute(
                "INSERT INTO node_embeddings(qualified_name, provider_identity, "
                "text_hash, vector, dimension, provider, model) "
                "VALUES(?, ?, ?, ?, ?, ?, ?)",
                (
                    "ignored.js::LegacyJs", "legacy-provider", "legacy-text-hash",
                    b"legacy-vector", 1, "legacy", "legacy-model",
                ),
            )
            conn.execute(
                "UPDATE metadata SET value=? WHERE key='schema_version'", ("2",)
            )
            conn.execute(
                "UPDATE metadata SET value=? WHERE key='impact_snapshot'",
                (
                    json.dumps({
                        "head": "legacy-head",
                        "snapshot": {
                            "nodes": [{"qualified_name": "ignored.js::LegacyJs"}],
                            "incoming": [],
                        },
                    }),
                ),
        ...
        upgraded = self.call(action="overview")
        self.assertTrue(upgraded["ok"], upgraded)
        self.assertEqual(upgraded["index"]["mode"], "full")
        self.assertEqual(upgraded["data"]["languages"], {"python": 1})
        legacy_search = self.call(action="search", query="LegacyJs")
        self.assertEqual(legacy_search["data"]["results"], [])

        with patch("pathlib.Path.home", return_value=self.home):
            conn = sqlite3.connect(_cache_path(self.repo))
            ...
            embedding_table = conn.execute(
                ...
                "AND name='node_embeddings'"
            ).fetchone()
            conn.close()

        self.assertEqual(schema_version, "3")
        self.assertNotIn("LegacyJs", snapshot)
        self.assertIsNone(embedding_table)
