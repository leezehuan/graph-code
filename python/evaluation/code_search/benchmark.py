...
def render_report(report: dict[str, Any]) -> str:
    lines = [
        ...
        f"运行时间：`{report.get('generated_at', 'n/a')}`。模型：`{report.get('models', {}).get('chat', 'n/a')}` / `{report.get('models', {}).get('embedding', 'n/a')}`。", "",
        "## 检索方法", "",
    ...
    for item in report["repositories"]:
        ...
        dirty = git.get("dirty")
        dirty_text = "是" if dirty is True else "否" if dirty is False else "未知"
        lines.append(
            f"| {item['name']} | {snapshot['source_files']} | "
            f"`{snapshot['corpus_sha256'][:12]}` | `{git.get('commit') or 'n/a'}` | "
            f"{dirty_text} |"
        )
    lines += ["", "## 检索效果", "", "| 仓库 / 方法 | 样本数 | 错误数 | 节点 Hit@5 | 节点 Recall@5 | 节点 MRR@10 | 文件 Hit@5 | 文件 Recall@5 | 文件 MRR@10 |", "|---|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for item in report["repositories"]:
    ...
    lines += ["", "## 关系检索", "", "| 仓库 / 方法 | 样本数 | 错误数 | Anchor 命中率 | 邻居 Recall@5 | 端到端成功率 | MRR@10 |", "|---|---:|---:|---:|---:|---:|---:|"]
    for item in report["repositories"]:
    ...
    lines += ["", "## 主链消融", "", "| 仓库 / 叠加步骤 | 指标 | 增量 | Bootstrap 95% 置信区间 |", "|---|---|---:|---:|"]
    for item in report["repositories"]:
    ...
    lines += ["", "## 冷启动延迟（毫秒）", "", "| 仓库 / 阶段 | 延迟 | 错误 | 更新节点数 |", "|---|---:|---|---:|"]
    for item in report["repositories"]:
        for phase, metrics in item.get("cold_start_ms", {}).items():
            ...
            lines.append(
                ...
                f"{metrics.get('error') or metrics.get('status') or '无'} | "
                f"{embedding.get('updated_nodes', 'n/a')} |"
            )
    lines += ["", "## 热搜索延迟（毫秒）", "", "| 仓库 / 方法 | 成功样本数 | P50 | P95 | 错误数 |", "|---|---:|---:|---:|---:|"]
    for item in report["repositories"]:
    ...
    lines += ["", "## 路由评测", "", "| 路由器 | 样本数 | 准确率 | 错误数 | P50（毫秒） | P95（毫秒） |", "|---|---:|---:|---:|---:|---:|"]
    ...
    for router, metrics in report["routing"].items():
        ...
        lines += [
            f"`{router}`（行 = 预标注，列 = 预测）", "",
            "| 预标注 / 预测 | " + " | ".join(f"`{value}`" for value in candidates) + " |",
            "|---|" + "---:|" * len(candidates),
        ]
        for label in labels:
            lines.append(
    ...
    if downstream:
        lines += ["### 固定下游效果", "", "| 路由器 | 预期 Hit@5 | 相对 oracle 的差值 | 样本数 |", "|---|---:|---:|---:|"]
        for router, metrics in downstream.items():
            lines.append(
                f"| `{router}` | {metrics['score']:.3f} | "
                f"{metrics['delta_vs_oracle']:.3f} | {metrics['samples']} |"
            )
        lines += ["", "该受控估计对所有路由器使用同一份仓库平均 Hit@5 计分表；预测到不兼容的路由类型时记零分。"]
    lines += ["", "## 生成质量", "", "| 仓库 / 方法 | 样本数 | 成功数 | 忠实度 | 答案相关性 | 错误数 | P50（毫秒） | P95（毫秒） |", "|---|---:|---:|---:|---:|---:|---:|---:|"]
    for item in report["repositories"]:
    ...
    lines += ["", "### 生成各阶段延迟（毫秒）", "", "| 仓库 / 阶段 | 成功样本数 | P50 | P95 |", "|---|---:|---:|---:|"]
    for item in report["repositories"]:
        generation = item.get("generation", {})
    ...
    if out.get("status") != "offline_mode":
        lines += ["", "## 库外问答", "", f"样本数：{out.get('samples', 0)}，成功数：{out.get('successful', out.get('samples', 0) - out.get('errors', 0))}，错误数：{out.get('errors', 0)}，LLM 正确性：{out.get('llm_correctness', 0):.3f}，embedding 余弦相似度：{out.get('embedding_cosine', 0):.3f}，综合分：{out.get('combined', 0):.3f}。"]
    lines += ["", "## 说明", "", "- `grep_file` 是文件级基线，因此不出现在节点检索方法表中。", "- `semantic`、`hybrid` 和 `hybrid_graph` 需要 `--live`；离线运行会记录 `offline_mode` 错误，不会静默删除样本。", "- 生成和库外问答中的失败样本按零分计入质量均值。", "- 本报告不包含源码正文、endpoint 或 API key。", ""]
    return "\n".join(lines)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.routing_only:
        ...
        (args.output / "latest.md").write_text(render_report(report), encoding="utf-8")
