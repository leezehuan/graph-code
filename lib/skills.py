...
class SkillStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.project_skills_dir = Path(project_skills_dir or self.project_root / "skills").resolve()
        self.user_skills_dir = Path(
            user_root or (Path.home() / ".langcode" / "skills")
        ).resolve()
        self.origin = origin
        ...
        self._read_paths: dict[Path, str] = {}

    def list(self, category: str | None = None) -> list[SkillDefinition]:
        skills, _ = self._discover()
        skills = tuple(s for s in skills if self.allowed_skill_names is None or s.name in self.allowed_skill_names)
        if category is None:
            return list(skills)
    ...
    def conflicts(self) -> list[str]:
        ...
        return sorted(name for name in conflicts if self.allowed_skill_names is None or name in self.allowed_skill_names)
    ...
    def view(self, name: str, file_path: str | None = None) -> dict[str, Any]:
        ...
        if file_path:
            ...
            try:
                if target.stat().st_size > MAX_SKILL_FILE_BYTES:
                    return {"success": False, "error": "Supporting file exceeds size limit"}
                raw_bytes = target.read_bytes()
                self._mark_read(target, raw_bytes)
        ...
        self._mark_read(skill_file, raw_bytes)
        return {
            "success": True,
            "name": skill.name,
    ...
    def format_index(self) -> str:
        ...
        lines.append("Use skill_view or load_skill to read instructions; skill_view also reads supporting files.")
        return "\n".join(lines)
    ...
    def manage(
        ...
        *,
        ...
        approved_user_path: str | None = None,
        enforce_user_permission: bool = False,
    ) -> dict[str, Any]:
        ...
        if self.allowed_skill_names is not None and name not in self.allowed_skill_names:
            return {"success": False, "error": f"Skill '{name}' is not allowed by the Agent Card."}
        try:
            with ExitStack() as locks:
                # Lock both roots in deterministic order; user skills may be shared by projects.
                for root in sorted({self.project_skills_dir, self.user_skills_dir}, key=str):
                    lock_path = project_cache(root, self.cache_dir) / "skills.lock"
                    lock = FileLock(lock_path)
                    deadline = time.monotonic() + 5
                    while True:
                        if self.cancel_event is not None and self.cancel_event.is_set():
                            return {"success": False, "error": "Skill review cancelled"}
                        try:
                ...
                if (action != "create" and selected and selected.source == "user"
                        and enforce_user_permission
                        and selected.skill_file != approved_user_path):
                    return {"success": False, "error": "Permission required for user skill write"}
                return self._manage_locked(handler, name=name, content=content, category=category,
                                           file_path=file_path, file_content=file_content,
                                           old_string=old_string, new_string=new_string, replace_all=replace_all)
        except (Timeout, OSError, ValueError) as exc:
            return {"success": False, "error": f"Skill update failed: {exc}"}

    def _manage_locked(self, handler, **kwargs) -> dict[str, Any]:
    ...
    def _parse_skill_file(
        ...
        try:
            if self._validate_within(skill_file, root) or skill_file.stat().st_size > MAX_SKILL_FILE_BYTES:
                return None
            raw = skill_file.read_text(encoding="utf-8")
            parsed = parse_frontmatter(raw)
            meta = parsed.meta
            name = meta.get("name") or skill_file.parent.name
            if not isinstance(name, str) or self._validate_name(name):
                return None
            ...
            if raw_tools is not None:
                if isinstance(raw_tools, list):
                    if any(not isinstance(item, str) for item in raw_tools):
                        return None
                    allowed_tools = raw_tools
                elif not isinstance(raw_tools, str):
                    return None
                elif raw_tools.startswith("["):
                    try:
                        decoded = json.loads(raw_tools)
                        if not isinstance(decoded, list) or any(not isinstance(item, str) for item in decoded):
                            return None
                        allowed_tools = decoded
            ...
            return SkillDefinition(
                name=name,
                description=str(meta.get("description", ""))[:MAX_DESCRIPTION_LENGTH],
    ...
    def _resolve(self, name: str) -> tuple[SkillDefinition | None, str | None]:
        if self.allowed_skill_names is not None and name not in self.allowed_skill_names:
            return None, f"Skill '{name}' is not allowed by the Agent Card."
        ...
        available = [skill.name for skill in skills
                     if self.allowed_skill_names is None or skill.name in self.allowed_skill_names][:20]
    ...
    def _delete(self, *, name: str, **_: Any) -> dict[str, Any]:
        ...
        if error:
            return {"success": False, "error": error}
        if self.origin == "background_review":
            # Removing a skill removes its attachments too. Every affected file
            # must have been read and must still match that read snapshot.
            for target in skill_dir.rglob("*"):
                if target.is_file():
                    error = self._background_write_error(skill, target, require_read=True)
                    if error:
                        return {"success": False, "error": error}
    ...
    def _background_write_error(
        ...
        if require_read:
            try:
                ...
                return "Skill changed since skill_view; read it again."
            if digest != self._read_paths.get(self._resolved(target)):
                return "Skill changed since skill_view; read it again before modifying it."
        return None
    ...
    def _directory_signature(self) -> tuple[tuple[str, int, str], ...]:
        ...
        for root in (self.user_skills_dir, self.project_skills_dir):
            if not root.is_dir():
                ...
                continue
            try:
                directories = [root, *(path for path in root.rglob("*") if path.is_dir() or path.name == "SKILL.md")]
            except OSError:
                directories = [root]
            for directory in directories:
                try:
                    if self._validate_within(directory, root):
                        continue
                    if directory.is_file() and directory.stat().st_size > MAX_SKILL_FILE_BYTES:
                        continue
                    signature.append((str(directory), directory.stat().st_mtime_ns,
                                      hashlib.sha256(directory.read_bytes()).hexdigest() if directory.is_file() else ""))
                except OSError:
                    continue
    ...
    def _mark_read(self, path: Path, content: bytes) -> None:
        if self.origin == "background_review":
            self._read_paths[self._resolved(path)] = hashlib.sha256(content).hexdigest()
...
def resolve_skill_prompt(skill: SkillDefinition, args: str) -> str:
    ...
    return prompt.replace("${LANGCODE_SKILL_DIR}", skill.skill_dir).replace("${CLAUDE_SKILL_DIR}", skill.skill_dir)
